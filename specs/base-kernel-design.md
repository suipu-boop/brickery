...
## 四、待动工项（实施清单，2026-08-15 更新）

| # | 事项 | 状态 |
|---|---|---|
| 1 | setup_wizard.py：安装引导（八家 API 预设 + 本地 GGUF 推荐下载 + 验证），写 config.json | 未做 |
| 2 | chat_ui.py：本地 web 聊天界面（工坊蓝图风，走引擎路由） | 未做 |
| 3 | ipc.py：启动扫描 home/bricks 激活积木（积木数非 0）；未配置引擎进安装引导 | 未做 |
| 4 | skill-library 改造为积木市场（brick-market） | 未做 |
| 5 | produce.py：出包默认带预置 7 积木；底座内核自包含 | 未做（用户：不急着打包） |
| 6 | 重出包到 /Applications，重打 DMG 到桌面验证 | 未做（用户：不急着打包） |

> 已拍板：底座内核写死（engine 双后端 + 记忆系统 8 能力 + 安装引导 + 聊天界面 + 连接器框架）；积木分层预置 7 / 按需 10 / 积木市场；界面工坊蓝图风，版面照搬 Shadeling 形态。
