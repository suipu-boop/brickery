> **历史存档（2026-09-16 方向修正后）**：本文档为方向修正（放弃组装步骤、底座即完整成品、GitHub Release 直接分发）前的规划/记录，其中 assembler / produce / e2e_produce / web 工作台 / 组装·产出链路相关内容均已失效，仅作历史参考。
...
### 5.1 引擎分层与资源协调（2026-08-25 补充）

**本地小模型属于「网络 API 之下」的幕后层**，与主引擎（聊天/推理）分层，不在安装引导页混入主引擎「本地模型选项」：
...
第二层 · 幕后引擎（背后工作：任务拆解/规划、记忆整理、语义嵌入）
  ├─ 默认 auto：先走网络 API，不可用时用本地小模型兜底
  └─ 本地小模型（轻量，qwen3.5-4b / bge 级别）
```

**资源冲突规则**：主引擎若为本地大模型（内存充足用户），幕后引擎**不得再独立加载本地小模型**（双模型驻留抢内存）：

- 主引擎 = 网络 API → 幕后引擎 auto：先 API，失败/离线时用本地小模型兜底（小模型轻量，无冲突）。
- 主引擎 = 本地大模型 → 幕后引擎默认**复用主引擎**（同一实例做规划/记忆/嵌入，不重复加载）；设置页可改为「独立本地小模型」或「关闭」。

**配套改动**：
- 引导页第一步分两区：主引擎选择（API / 本地大模型）+ 幕后引擎配置（auto / 仅本地小模型 / 关闭 + 小模型推荐下载），文案不再用「隐私兜底」暗示本地为主引擎兜底。
- 运行时设置页：幕后引擎独立配置区 + 模型目录下载按钮 + 顶栏双引擎状态。
- 代码偏差已收敛（2026-08-25）：`engine_router.py` 已去除聊天/推理的本地自动降级兜底（complete/run_turn 只走用户显式选择的后端，API 失败直接上浮报错），与本节拍板一致；单测 17 例全过。
