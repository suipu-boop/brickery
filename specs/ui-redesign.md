...
**现状问题**：本地小模型（规划/记忆/嵌入的幕后引擎）功能代码齐全，但界面上不可见：

| 位置 | 现状 | 问题 |
|------|------|------|
| setup_wizard 第一步 | `#localPanel` 默认 `display:none`，须把「后端模式」下拉切到「本地 GGUF」才显示模型输入与下载列表 | 用户不切换就看不到任何本地模型操作；「智能推荐下载」也未做成显眼推荐 |
...
**改造方案**（让"幕后引擎"常态可见、可操作）：

1. **setup_wizard 第一步**：取消折叠——「API 配置」与「本地模型推荐」上下分区同时可见；本地模型区做成推荐卡片（标注"按内存推荐"、已安装/下载中状态、下载按钮），文案从"隐私兜底"改为"幕后引擎：规划/记忆/语义检索"。
2. **chat_ui 设置页**：`#secLocal` 不再随 backend 隐藏，改为始终显示；模型目录列表加「下载」按钮（复用引导页 /api/download 同款逻辑）。
3. **顶栏引擎状态**：本地模型状态行可点击，跳转到设置页模型管理区；未就绪时给出「去下载」指引。
4. **文案统一**：界面用「幕后引擎」替代「兜底」表述，明确本地小模型只做规划类任务（任务拆解、记忆整理、语义嵌入），不做聊天回复与推理。
